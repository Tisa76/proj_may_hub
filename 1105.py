...
#Коллекции.
some_list = [2, 3, 4, 'cow', 'car', 765, 'negros'] #[] пишется чтобы, создать формат list без него будет формат tuple
# формат tuple пишется через () а формат set пишется через {} и данные вводятся через запятую
# если, вводить данные (или ничего не вводить а просто {}) через (:), (приммер {'car: 7,}) то получим формат dict
...
print(len(some_list)) # функция показывает сколько элементов в списке

some_list.append(76) # функция добавление в самый конец
print(some_list)

print(len(some_list))
#some_list.pop(0) # функция просто удаляет элемент из списка
deled = some_list.pop(0) # так превращаем в переменную и может через 'print' показать что удалили.
print(some_list)
print(len(some_list))
print(deled)